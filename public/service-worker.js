...
// ---------------------------------------------------------------------------
// Max number of dynamically-cached entries (app-shell URLs are always kept).
  ...
  caches.open(cacheName).then(cache => {
    cache.keys().then(keys => {
      if (keys.length > maxItems) {
        // Delete oldest entries until we're within the limit
...
self.addEventListener('fetch', event => {
  ...
  // Skip caching for large assets (> ~500 KB) — e.g. audio files, user images
  // that aren't in the shell list. They can always be fetched from the network.
  event.respondWith(
    caches.match(event.request).then(cachedResponse => {
      ...
      return fetch(event.request).then(networkResponse => {
        ...
        // Only cache same-origin responses that aren't API calls
        if (!event.request.url.startsWith(self.location.origin + '/api/')) {
          const responseToCache = networkResponse.clone();
          caches.open(CACHE_NAME).then(cache => {
            cache.put(event.request, responseToCache);
            trimCache(CACHE_NAME, DYNAMIC_CACHE_MAX);
          });
        }
        return networkResponse;
      });
