...
const App = () => {
    ...
    const applyTheme = useCallback((selectedTheme: 'light' | 'dark' | 'glass') => {
        ...
        if (api) api.put('config/theme.json', selectedTheme).catch(console.error);
    }, []);
    ...
    useEffect(() => {
        if (isAuthenticated && isFirebaseConfigured && api) {
            ...
            api.get('config/theme.json').then(themeFromDb => {
                if (themeFromDb === 'light' || themeFromDb === 'dark' || themeFromDb === 'glass') {
                    const localTheme = localStorage.getItem('theme');
                    if (themeFromDb !== localTheme) {
                        const root = window.document.documentElement;
                        root.classList.remove('light', 'dark', 'glass');
                        root.classList.add(themeFromDb);
                        localStorage.setItem('theme', themeFromDb);
                        setTheme(themeFromDb);
                    }
                }
            }).catch(console.error);
        }
    ...
    useEffect(() => {
        let onlineToastTimer: ReturnType<typeof setTimeout> | null = null;
        ...
        const handleOnline = () => {
            ...
            if (!firstLoadRef.current) {
                ...
                onlineToastTimer = setTimeout(() => setShowOnlineToast(false), 3000);
            }
        };
        const handleOffline = () => {
            setIsOnline(false);
            setShowOnlineToast(false);
            if (onlineToastTimer) { clearTimeout(onlineToastTimer); onlineToastTimer = null; }
        };
